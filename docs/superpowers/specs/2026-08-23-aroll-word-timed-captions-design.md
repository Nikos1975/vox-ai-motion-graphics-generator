...
Local faster-whisper becomes the canonical and only A-roll ASR engine in this change. The target PC has 24 GB system RAM and 2 GB GPU VRAM, so every built-in profile uses CPU and does not depend on the GPU. The default accurate multilingual profile is `large-v3-turbo` / `cpu` / `int8`; the light fallback is `small` / `cpu` / `int8`. English-only users may explicitly select a Distil-Whisper model supported through faster-whisper with `cpu` / `int8`; Distil-Whisper is not the multilingual default. Model, device, and compute type remain configurable. Real validation benchmarks the accurate profile on a short A-roll fixture. If its measured wall time is unreasonable for this PC, the architecture stays unchanged and the operational default becomes the measured light profile, with the reason recorded. This PR does not add original `openai-whisper`, Qwen3-ASR, whisper.cpp, llama.cpp, vLLM, or bitsandbytes. The existing MuAPI Whisper path is superseded rather than retained as a second backend because it adds a paid remote request, has an incompatible provider result contract, and would reintroduce parallel transcription behavior. The former `source_audio_url` field is not preserved because neither `aroll_clips.py` nor `aroll_assemble.py` consumes it. MuAPI remains unchanged for A-roll visual generation.
...
Real local validation uses an ignored copy of a safe existing A-roll project, or a locally generated short talking-head-style fixture if none exists. It benchmarks faster-whisper `large-v3-turbo`, `device=cpu`, and `compute_type=int8`, recording source duration, transcription wall time, segment and word counts, cache reuse, approximate peak RAM when practical, timestamp quality, and obvious transcription errors. If the accurate profile is unreasonably slow, validation repeats with `small` / `cpu` / `int8` and makes that measured profile the operational default without redesigning the architecture. It renders word and off variants without MuAPI generation, probes video/audio streams and duration, and inspects representative frames for caption burn-in. Generated media, model cache data, source media, secrets, and the ignored validation project are not committed.
